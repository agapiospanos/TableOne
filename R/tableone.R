...
#' @param tableone.col.names (Character) (optional) a vector for the column names of the exported table. Default are: c(' ', 'Treatment Group', 'Control Group', '95\% CI', 'Z-value', 'p-value')
...
tableone <- function(columns, var.names, percentages, group.col.name, control.value, treatment.value, path = NULL, export.path = NULL, sheet = NULL, tableone.col.names = NULL, export.filename = NULL) {
    ...
    table.to.export <- data.frame( matrix( ncol = 6, nrow = length(var.names) ) )

    # checking if the user specified custom column names for the exported table
    if (is.null(tableone.col.names)) {
        tableone.col.names <- c(' ', 'Treatment Group', 'Control Group', '95% CI', 'Z-value', 'p-value')
    } else {
        if (length(tableone.col.names) < 6) {
            stop('you must provide a vector with 6 column names for the argument tableone.col.names. Use " " inside the vector to keep empty column names')
        } else {
